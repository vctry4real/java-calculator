...
class History{
    ...
    public History(){
        window = new JFrame("Calculator History");
        ...
        text = new JTextArea("History" );
        text.setBounds(0, 0, 300, 70);
        text.setEditable(false);
        text.setBackground(Color.WHITE);
        text.setFont(new Font("Comic Sans MS", Font.PLAIN, 25));
        //window.add(text);
    ...
    public void setHistory(List<String> calcHistory){
        if(calcHistory.size()==0) return;
        history=calcHistory.stream()
    ...
    */
    ...
    public double calc(double x, String input, char opt) {
        ...
        switch (opt) {
            case '+':{
                history.add(x+"+"+y+"="+(x+y));
                historyInterface.setHistory(history);
                return x + y;
            }
